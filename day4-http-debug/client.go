...
// NewHTTPClient new a Client instance which use http as transport protocol
...
// XDial calls different functions to connet to a RPC server according to the first parameter of rpcAddr.
// rpcAddr is a general format (protocol@addr) to represent a rpc server
// eg, http@10.0.0.1:7001, tcp@10.0.0.1:9999, unix@/tmp/goorpc.sock
func XDial(rpcAddr string, opts ...*Option) (client *Client, err error) {
	parts := strings.Split(rpcAddr, "@")
	if len(parts) != 2 {
		return nil, fmt.Errorf("rpc client: wrong rpc address format '%s', expect protocol@addr", rpcAddr)
	}
...
// Call invokes the named function, waits for it to complete within given timeout,
// and returns the error.
func (client *Client) Call(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	call := client.Go(serviceMethod, args, reply, make(chan *Call, 1))
	select {
	case <-ctx.Done():
		_ = client.removeCall(call.Seq)
		return fmt.Errorf("rpc client: call timeout: %s", ctx.Err().Error())
	case call := <-call.Done:
		return call.Error
	}
}
